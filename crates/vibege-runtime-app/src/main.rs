...
use vibege_scene::scene::{SceneManager, SceneContext, SceneAction};
use vibege_scene::scenes::boot_scene::BootScene;
use vibege_scene::scenes::game_scene::GameScene;
...
fn main() -> anyhow::Result<()> {
    ...
    let _game_source: Option<String> = if has_game {
    ...
    if start_visible { info!("Window will be visible on startup"); }
    else { info!("Runtime started in background — tray icon active"); }
    if cfg.is_first_run() { info!("First run detected"); }
    ...
    if _tray_handle.is_some() { info!("System tray active"); }

    // GPU Renderer
    let (w, h) = { let size = window.inner_size(); (size.width, size.height) };
    ...
    if audio.is_some() { info!("Audio system ready"); }
    ...
    }
    {
        runtime_table.set("list_installed", platform_lua.create_function(move |lua, ()| {
            let game_dirs = vibege_config::installed_games_dir();
    ...
    event_loop.run(move |event, elwt| {
        match event {
            Event::WindowEvent { event, .. } => {
                ...
                match event {
                    WindowEvent::CloseRequested => { info!("Window closed"); elwt.exit(); }
                    WindowEvent::Focused(true) => { info!("Focus gained"); }
                    WindowEvent::Focused(false) => { info!("Focus lost"); }
                    _ => {}
                }
            }
            Event::AboutToWait => {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    let (mod_ctrl, mod_shift, mod_alt) = (
                        k_mod.contains("ctrl"), k_mod.contains("shift"), k_mod.contains("alt"),
                    );
                    unsafe {
                        ...
                        if mod_match && key < 0 { vibege_tray::request_toggle(); }
                    }
                }
                ...
                if has_game && !state_restored {
                    ...
                    if let Some(snap) = suspension.list_snapshots().first().cloned() {
                        if let Ok(_snapshot) = suspension.resume(&snap.id) {
                ...
                let action = match scene_manager.update(&mut scene_ctx, dt) {
                    Ok(a) => a,
                    Err(e) => { warn!("Scene update: {e}"); SceneAction::Exit }
                };
                ...
                if let Some(game_name) = pending_switch.lock().unwrap().take() {
                    let src = embedded_games.get(game_name.as_str()).copied().unwrap_or("");
                    if src.is_empty() {
                        let path = PathBuf::from(&game_name);
                        if path.exists() {
                            if let Ok(s) = std::fs::read_to_string(&path) {
                                let gs = GameScene::new(s, Arc::clone(&renderer), Arc::clone(&input), audio_clone.clone());
                                let _ = scene_manager.push(Box::new(gs), &mut scene_ctx);
                            }
                        }
                    } else {
                        let gs = GameScene::new(src.to_string(), Arc::clone(&renderer), Arc::clone(&input), audio_clone.clone());
                        let _ = scene_manager.push(Box::new(gs), &mut scene_ctx);
                    }
                }
                ...
                let action = match scene_manager.render(&mut scene_ctx) {
                    Ok(a) => a,
                    Err(e) => { warn!("Scene render: {e}"); SceneAction::Exit }
                };
                if let Err(e) = scene_manager.apply(action, &mut scene_ctx) {
                    warn!("Navigation: {e}");
                }
                ...
                // Present
                if let Err(e) = renderer.render() { error!("GPU render: {e}"); }

                input.lock().unwrap().end_frame();
                window.request_redraw();

                if scene_manager.is_empty() { elwt.exit(); }
            }
            _ => {}
        }
    }).map_err(|e| anyhow::anyhow!("Event loop: {e}"))?;
