...
use vibege_scene::scene::{SceneManager, SceneContext};
...
fn main() -> anyhow::Result<()> {
    ...
    if start_visible { info!("Window visible on startup"); }
    else { info!("Runtime started in background"); }
    ...
    // ── GPU Renderer ──
    let (w, h) = { let s = window.inner_size(); (s.width, s.height) };
    ...
    let mut _suspension = SuspensionEngine::with_config(SuspensionConfig {
        snapshot_dir: snap_dir, enable_compression: false, ..Default::default()
    })?;
        ...
        match event {
            ...
            Event::AboutToWait => {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    let (mc, ms, ma) = (k_mod.contains("ctrl"), k_mod.contains("shift"), k_mod.contains("alt"));
                    unsafe {
                        ...
                        let vk = match k_key.as_str() {
                            ...
                            "h" => 0x48, "space" => 0x20, "tab" => 0x09, _ => 0x56,
                        };
                        let key = GetAsyncKeyState(vk);
                        if (!mc || ctrl < 0) && (!ms || shift < 0) && (!ma || alt < 0) && key < 0 {
                ...
                let action = match scene_manager.update(&mut scene_ctx, dt) {
                    Ok(a) => a,
                    Err(e) => { warn!("Scene update: {e}"); return; }
                };
                ...
                let action = match scene_manager.render(&mut scene_ctx) {
                    Ok(a) => a,
                    Err(e) => { warn!("Scene render: {e}"); return; }
                };
                if let Err(e) = scene_manager.apply(action, &mut scene_ctx) {
                    warn!("Navigation: {e}");
                }
                ...
                // Present
                if let Err(e) = renderer.render() { error!("GPU: {e}"); }
