    ...
    fn current_games(&self) -> Vec<InstalledGame> {
        ...
        if self.filter_favorites {
            let favs = self
                .manager
                .collections
                .by_kind(crate::library::models::CollectionKind::Favorites);
...
impl Scene for LibraryScene {
    ...
    fn on_update(&mut self, ctx: &mut SceneContext, _dt: f64) -> SceneResult {
        let inp = InputState::new(
            &ctx.input,
            &[
                "up", "down", "enter", "escape", "r", "f", "delete", "c", "u", "v", "s", "x", "a",
            ],
        );
                        ...
                        DisplayMode::List => {
                            for (i, game) in games.iter().enumerate() {
                                ...
                                let details = format!(
                                    ...
                                    game.version,
                                    game.author,
                                    format_file_size(game.size_bytes),
                                    format_duration(game.total_play_time_secs),
                                    game.play_count
                                );
