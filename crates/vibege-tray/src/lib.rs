...
pub fn should_show_launcher() -> bool { SHOW_LAUNCHER.swap(false, Ordering::SeqCst) }
pub fn should_toggle_overlay() -> bool { TOGGLE_OVERLAY.swap(false, Ordering::SeqCst) }
pub fn request_toggle() { TOGGLE_OVERLAY.store(true, Ordering::SeqCst); }
pub fn should_quit() -> bool { QUIT.load(Ordering::SeqCst) }

pub fn start() -> Option<std::thread::JoinHandle<()>> {
    #[cfg(windows)]
    { Some(start_windows()) }
    #[cfg(not(windows))]
    { tracing::warn!("Tray not supported on this platform"); None }
}

#[cfg(windows)]
fn start_windows() -> std::thread::JoinHandle<()> {
    std::thread::Builder::new().name("vibege-tray".into()).spawn(tray_loop).expect("tray thread")
}
...
fn tray_loop() {
    ...
    unsafe {
        ...
        let wc = WNDCLASSW {
            style: 0, lpfnWndProc: Some(tray_proc),
        ...
        let hwnd = CreateWindowExW(0, class.as_ptr(), to_wide("VG").as_ptr(),
            0, 0, 0, 0, 0, 0isize, 0isize, inst as isize, std::ptr::null());
        ...
        for (i, &c) in tip.iter().enumerate().take(128) { nid.szTip[i] = c; }
        ...
        match msg {
            WM_TRAY => {
                ...
                if cmd == WM_RBUTTONUP || cmd == WM_LBUTTONUP {
                    ...
                    if menu != 0 {
                        AppendMenuW(menu, MF_STRING, IDM_LAUNCHER as usize, to_wide("Open Game Store\0").as_ptr());
                        AppendMenuW(menu, MF_STRING, IDM_TOGGLE as usize, to_wide("Show/Hide Overlay\0").as_ptr());
                        AppendMenuW(menu, MF_SEPARATOR, 0, std::ptr::null());
                        AppendMenuW(menu, MF_STRING, IDM_QUIT as usize, to_wide("Quit\0").as_ptr());
